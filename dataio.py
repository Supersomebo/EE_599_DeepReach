...
def get_mgrid(sidelen, dim=13):
    ...
    if dim == 2:
        ...
        pixel_coords = np.stack(np.mgrid[:sidelen[0], :sidelen[1], :sidelen[2], :sidelen[3], :sidelen[4], :sidelen[5],
                                :sidelen[6], :sidelen[7], :sidelen[8], :sidelen[9], :sidelen[10], :sidelen[11],
                                :sidelen[12]], axis=-1)[None, ...].astype(np.float32)
        ...
        pixel_coords[..., 10] = pixel_coords[..., 10] / (sidelen[10] - 1)
        pixel_coords[..., 11] = pixel_coords[..., 11] / (sidelen[11] - 1)
        pixel_coords[..., 12] = pixel_coords[..., 12] / (sidelen[12] - 1)
    else:
...
class ReachabilityQuadrupedSource(Dataset):
    def __init__(self, numpoints,
                 collisionR=0.25, velocity=3, omega_max=1.1,
                 pretrain=False, tMin=0.0, tMax=0.5, counter_start=0, counter_end=100e3,
